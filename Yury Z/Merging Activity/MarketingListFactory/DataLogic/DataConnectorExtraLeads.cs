...
namespace MarketingListFactory.DataLogic
{
    ...
    {
        public DataConnectorExtraLeads(Stream sourceFileStream)
        {
            ...
            _workSheet = sourceExcelPackage.Workbook.Worksheets[dataSheetName] ??
                         sourceExcelPackage.Workbook.Worksheets[1];
        }
        public override List<string> GetHeaderValues()
        {
            var result = new List<string>();
            var lastColumnIndex = _workSheet.Dimension.End.Column;
            for (int rowIndex = 1; rowIndex <= 4; rowIndex++)
            {
                for (int columnIndex = 1; columnIndex <= lastColumnIndex; columnIndex++)
                {
                    result.Add(_workSheet.Cells[rowIndex, columnIndex].SafeStringValue());
                }
                if (result.Any())
                {
                    break;
        ...
        protected override Dictionary<Columns, int> ReadHeaders()
        {
            ...
            for (int rowIndex = 1; rowIndex <= 4; rowIndex++)
            {
                for (int columnIndex = 1; columnIndex <= lastColumnIndex; columnIndex++)
                {
                    switch (_workSheet.Cells[rowIndex, columnIndex].SafeStringValue())
                    {
                        case "ID":
                        ...
                        case "Town":
                            ...
                            break;
                        case "County":
                            result[Columns.ArchitectCompanyCounty] = columnIndex;
                            break;
                        case "Post Code":
